...
app.post(
  "/json-utils/api/v1/json2yml/indent/:indent",
  Express.text({ type: "*/*" }),
  (req, res) => {
...
app.post(
  "/json-utils/api/v1/yml2json/indent/:indent",
  Express.text({ type: "*/*" }),
  (req, res) => {
...
app.post(
  ...
  (req, res) => {
    try {
      // parse YML string
      const nativeObject = YMLJS.parse(req.body);
...
app.post(
  "/json-utils/api/v1/yml/beautify/indent/:indent",
  Express.text({ type: "*/*" }),
  (req, res) => {
    try {
      const ymlStr = YML.stringify(YML.parseDocument(req.body), {
        indent: Number(req.params.indent),
      });
      return res.send(ymlStr);
    } catch (e) {
      res.header("Content-Type", "application/json");
      const errorResponse = {
        message: e.message,
...
app.post(
  "/json-utils/api/v1/xml2json/indent/:indent",
  Express.text({ type: "*/*" }),
  (req, res) => {
    const xml = req.body;
...
app.post(
  "/json-utils/api/v1/xml2yml/indent/:indent",
  Express.text({ type: "*/*" }),
  (req, res) => {
    const xml = req.body;
    XML2JS.parseString(xml, (err, result) => {
...
app.post(
  "/json-utils/api/v1/xml/beautify/indent/:indent",
  Express.text({ type: "*/*" }),
  (req, res) => {
    const xml = req.body;
    const indent = Number(req.params.indent);
...
app.post(
  "/json-utils/api/v1/json2xml/indent/:indent",
  Express.text({ type: "*/*" }),
  (req, res) => {
